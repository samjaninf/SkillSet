...
const forceLayoutVisualize = function (rawLinks) {
  // contain data for nodes
  var nodes = {};
  var links = rawLinks.map((link) => {
    return {
      source: link.source,
      target: link.target,
      value: +link.value
  ...
  var root = d3.select("#visualization-root");
  var width = Math.max(768, root.node().clientWidth || 1024);
  var height = 768;
